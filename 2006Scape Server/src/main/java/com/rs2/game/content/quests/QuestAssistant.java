...
import com.rs2.game.content.quests.impl.DesertTreasure;
...
public class QuestAssistant {
	...
	public static void sendStages(Player player) {
		...
		if (player.desertT == 0) {
			player.getPacketSender().sendString("Desert Treasure", 12852);
		} else if (player.desertT == 1) {
			player.getPacketSender().sendString("@yel@Desert Treasure", 12852);
		} else if (player.desertT == 2) {
			player.getPacketSender().sendString("@gre@Desert Treasure", 12852);
		}
		if (player.gertCat == 0) {
	...
	public enum Quests {
		...
		FISHING_CONTEST(28191, 7359, "Fishing Contest", true),
		...
		ICTHLARIN(17156, 4508, "Icthlarin's Little Helper", true),
		...
		REGICIDE(33128, 8576, "Regicide", true),
		...
		SOULS_BANE(28250, 15098, "A Soul's Bane", true),
	...
	public static void questButtons(Player player, int buttonId) {
		...
		case 28172:
			...
			break;
		case 50052:
			DesertTreasure.showInformation(player);
			break;
		case 28173:
